  ...
  switch (action.type) {
    ...
    case types.FLIP_TILE: {
      ...
      return produce(state, (updatedState) => {
        const tileToFlip = action.payload.tile;
        const { id: index } = tileToFlip;
        updatedState.tiles = flipTile(state, tileToFlip);
        updatedState.flipped.push(updatedState.tiles[index]);

        const twoTilesAreFlipped = updatedState.flipped.length === 2;
        if (twoTilesAreFlipped) {
          updatedState.tiles = lockTiles(updatedState.tiles);
        }
      ...
      // let tempState = {
      //   ...state,
      //   tiles: flipTile(state, action.payload.tile),
      // };
      ...
      //   draft.tiles = flipTile(state, action);
      // });
